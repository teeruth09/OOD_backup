...
class BST:
    ...
    def _build_tree_string( 
        ...
        if l_box_width > 0:
            ...
            line2.append(" " * (l_root))
            line2.append("_" * (l_box_width - l_root)+"/")
        ...
        # Remove underscores above nodes with no children
        if root.left is None:
            for i in range(len(new_box) - 2, -1, -1):
                if new_box[i].count("_") > 0:
                    new_box[i] = new_box[i].replace("_", " ")
                else:
                    break
